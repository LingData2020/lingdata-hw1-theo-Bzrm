...
About 25% of all values exceed 5. 5 observations out of 20 which is 25%.
b. Indicate the interval where the median of this sample can lie. Explain your answer.
That interval will be [-5;-0] because it will be in the middle of values sorted in ascending order. The median wiil be in the interval where 10-11th observations are.  
c. How the histogram will change if we add an element $7$ to the sample? Explain your answer. 
The frequency of interval [5; 10] will climb to 5 and the proportion of values in the sample that exceed $5$ will be a 0.28.  
### Problem 2
...
The first one because the some of it's reaction time values are lower then 400 or higher then 600 while the second sample stays in an 
interval 400-600.
\newpage
...
```
Which of the samples has a larger variance? Explain your answer. 
The first one because interval in which values are put is about 20, while in the second case it's just about 4.  
...
The frequences to the left of the mark of 35 mistakes are much higher then values to the right
...
```
Explain your answer below:
I think the sum of values which ilustrate more than 10 mistakes level will be much higher, even if border frequencies (about 10 mistakes) is quet significant. 
...
```
Explain your answer below: 
Because frequencies of observations to the left of $20$ is much higher than frequencies to the right of it, but the number of observations left to $20$ is small while to the right there are plenty of observations. So $20$ will be somewhere in the middle of the sorted sample, where median is. 
...
```
Explain your answer below:
P-value = 1 000 000 * (1/1 000 00)^1 000 000 or a little bit higher, but still it would not become > 0.05, so we have to reject null hypothesis
...
```{r}
words.relfreq = words.freq / sum(words.freq)
words.relfreq
```
...
```{r}
library(readr)
df.rhyme <- data.frame(read.delim('https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/poetry_last_in_lines.csv', sep = '\t', header = TRUE))
sample_1920 <- subset(df.rhyme, Decade == '1920s', select = 'UPoS')
str(sample_1920)
#the size is 277 obs.
summary(sample_1920)
#39 VERBS
...
```{r}
binom.test(39, 277, p = 0.17,alternative = c("two.sided", "less", "greater"),conf.level = 0.95)
```
...
```
p-value is 0.2297 > 0, 05 which means we cannot reject null hypothesis.
...
```{r}
sample_1820 <- subset(df.rhyme, Decade == '1820s', select = 'UPoS')
str(sample_1820)
#the size is 87 obs. of  1 variable
summary(sample_1820)
#15 VERBS
```

```{r}
binom.test(15, 87, p = 0.17,alternative = c("two.sided", "less", "greater"),conf.level = 0.95)
```

Write down your general conlusions about data provided for both 1920s and 1820s data.
```
Again we cannot reject null hypothesis. P-value of second binominal test is 0.8873 which is again more then 0.05 and also the sample probability of succes is about 17.
So considering the results of two test we should admit that probability of VERB in random verse isn't higher or lower than in any other position ('in general')
```
...
```
H1: true mean.vowel duration > 73
    true mean.vowel duararion < 73
```
...
```{r}
library(readr)
df.ice <- data.frame(read.csv('https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/icelandic.csv', header = TRUE))
df.ice
sample <- subset(df.ice, speaker == 'shg05', select = 'vowel.dur' )
t.test(sample, mu = 73)
```
...
Interpret results. 
```
p-value > 0.05 which means we cannot reject null hypothesis with confidence level of 5% (mean vowel duration can be thought as 73ms)
