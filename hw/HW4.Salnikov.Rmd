...
```{r}
summary(unique(mono_socio$ParticipantID))
```
$30$

2. How many of them are males and females?
```{r}
sex_df <- filter(mono_socio, QuestionCategory == 'sex')
sex <- factor(sex_df$Responce)
summary(sex)
```
```female - 19, male - 11```
...
```{r}
education_df <- filter(mono_socio,  QuestionCategory == 'education')
unique(education_df$Responce)
```
```College, UniversityDegree, Postgraduate, PhDongoing```

4. How many participants of each education levels are present?
```{r}
education <- factor(education_df$Responce)
summary(education)
```
```College - 3, UniversityDegree - 12, Postgraduate - 14, PhDongoing - 1```
...
```{r}
handedness_df <- filter(mono_socio,  QuestionCategory == 'handedness')
handedness <- factor(handedness_df$Responce)
summary(handedness)
```
left - 2
right - 28
...
```{r}
library(ggplot2)
plot <- ggplot(mono, aes(mono$RT)) + geom_density()
plot
```
...
```{r}
library(ggplot2)
ggplot(mono, aes(mono_log$RTlog)) + geom_density()
```
...
```Right tail is still too long and the plot is skewed to the left a little bit but now it seems to be closer to normal distribution. However there are still some oitliers at the $4$ mark (axis x).```
...
```{r}
library(dplyr)
RTlog_dif <-mean(mono$RTlog)-3*sd(mono$RTlog)
 mono1 <- filter(mono, RT > 600, TypeOfStimuli != "FillerAcceptable" & TypeOfStimuli != "FillerUnacceptable", sd(RTlog) < RTlog_dif)
...
summary <- mono1 %>%
  ...
  ggplot(summary, aes(x = TypeOfStimuli, y = Freq, fill = AcceptabilityJ)) +
  geom_col(color = 'black',position = "dodge") + facet_wrap(vars(WordOrder))
...
summary2 <- mono1 %>%
  ...
  ggplot(summary2, aes(x = WordOrder, y = Freq, fill = AcceptabilityJ)) +
...
summary3 <- mono1 %>%
  ...
  ggplot(summary3, aes(x = AcceptabilityJ, y = RTlog, fill = WordOrder)) +
  geom_violin(color = "black", position = "dodge") +  facet_wrap(vars(WordOrder)) + 
...
```{r}
library(ggpubr)
ggline(summary3, x = 'AcceptabilityJ', y = 'RTlog', 
...
library(tidyr)
mean_rtlog_long %>%
  pivot_wider(names_from = WordOrder, values_from = RTlog) -> mean_rtlog
#mean_rtlog
...
```{r}
x = mean_rtlog$Congruent
#print(x)
y = mean_rtlog$Incongruent
#print(y)
t.test(x,y)
```
...
```Word order (Congruent or Incongruent) doesn't significantly affect reaction speed.```  
...
mean_rtlog  %>%
  mutate(diff = (Congruent - Incongruent)) -> mean_rtlog1
...
```Word order (Congruent or Incongruent) does affect reaction speed```. 


How can you interpret difference with the result of 2.3?

```Maybe the reason for such difference is that in the first case we were using mean-reaction time for Congruent and Incongruent word order. However comparing reaction time for every pair we've got more accurate results which show that there are signifficant effect of word order to reaction speed.``` 
...
```{r}
library(dplyr)
incong_rtlog <- filter(mono1, WordOrder == 'Incongruent') %>% group_by(ParticipantID, TypeOfStimuli, RTlog) -> incong_rtlog
...
```We should use ANOVA test cause this test allows us to test dependencies of groups by analyzing their mean values (in our case analyzing several conditions).```
...
```Yes, there is significant difference between groups hightlighted with ***. It can be observed through Pr(>F) parameter, which is much lower, than 0.05.``` 
...
```SubjectiveComment-Material-Shape-Color and SubjectiveComment-Material-Size-Nationality pairs have statistically significant difference between logarithms of reaction time (their p-value is lower than 0.005).```
